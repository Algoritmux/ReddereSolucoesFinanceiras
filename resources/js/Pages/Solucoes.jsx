...
function SolutionCard({ sol, index }) {
    ...
    return (
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center py-14 border-b border-rd-gold/10 last:border-b-0 ${!isEven ? 'md:[&>*:first-child]:order-2' : ''}`}>
            <div>
            ...
            <div className="relative aspect-[5/4] bg-[rgba(15,85,100,.35)] border border-rd-gold/18 rounded flex items-center justify-center overflow-hidden">
                <div className="absolute inset-0 bg-grid-gold-sm" />
                <div className="relative w-16 h-16 border-[1.5px] border-rd-gold flex items-center justify-center rounded">
                    <div className="w-7 h-7 bg-rd-gold" style={{ clipPath: 'polygon(50% 0,100% 50%,50% 100%,0 50%)' }} />
                </div>
            </div>
        </div>
    );
...
export default function Solucoes() {
    ...
    return (
        <MainLayout>
            ...
            {/* FILTROS */}
            <div className="bg-rd-dark border-y border-rd-gold/16 px-8 py-5 sticky top-9 z-50">
                <div className="max-w-[1240px] mx-auto flex flex-wrap gap-2">
                    {CATEGORIES.map((cat) => (
                        <button
                            key={cat}
                            onClick={() => setActiveCategory(cat)}
                            className={`text-[13px] font-semibold px-4 py-2 rounded-sm border transition-all cursor-pointer ${
                                activeCategory === cat
            ...
            {/* LISTA DE SOLUÇÕES */}
            <section className="bg-rd-primary px-8 py-10">
            ...
            <section className="bg-rd-teal py-20 px-8 relative overflow-hidden">
                ...
                <div className="max-w-[780px] mx-auto text-center relative">
                    ...
                    <div className="flex flex-wrap gap-4 justify-center mt-8">
                        <Link
                            href="/consultor"
                            className="inline-flex items-center gap-2 bg-rd-gold text-rd-primary font-bold text-[15px] px-7 py-4 rounded-sm transition-all hover:bg-rd-gold-light hover:-translate-y-0.5"
                        >
                            Usar o Consultor On-line →
                        </Link>
                        <Link
                            href="/contato"
                            className="inline-flex items-center gap-2 bg-transparent text-rd-cream font-semibold text-[15px] px-7 py-4 border border-rd-cream/30 rounded-sm transition-all hover:border-rd-gold hover:text-rd-gold"
                        >
